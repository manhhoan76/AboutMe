    ...
    <%@include file="/templates/taglib.jsp" %>
      ...
      <section id="main-content">
		  ...
		  <div class="row">
				...
				<div style="margin-left: 16px; margin-top: -14px; margin-bottom: 5px; display: inline;">
                      ...
                      <form class="navbar-form" style="display: inline-flex;" action="${pageContext.request.contextPath }/admin/network/search">
                           <div>
                            <input class="form-control" placeholder="Search" type="text" name="key">
                      </div>
                        </form>
                       <a class="btn btn-success btn-sm"  href="${pageContext.request.contextPath }/admin/network/add" title="Bootstrap 3 themes generator" >Thêm</a>
				</div>
			</div>
			<c:choose>
				<c:when test="${msg == 1 }">
                      ...
                      <section class="panel">
                          <table class="table table-striped table-advance table-hover">
                           <tbody>
                              <tr>
								<th><i class=""></i> ID</th>
                                 <th><i class="icon_profile"></i> Network</th>
                                 <th><i class="icon_mail_alt" ></i> Link</th>
                                 <th><i class="icon_calendar"></i> Ngày tạo</th>
                                 <th><i class="icon_cogs"></i> Action</th>
                              </tr>
                              <c:forEach items="${listNetwork }" var="objnetwork">
                              <tr>
								 <td>${objnetwork.id_network }</td>
                                 <td>${objnetwork.name }</td>
                                  <td><a href="${objnetwork.link }">${objnetwork.link }</a></td>
                                 <td>${objnetwork.date_create }</td>
                                 <td>
                                  <div class="btn-group">
                                  <ul class="nav nav-tabs">
                                  	<li><a class="btn btn-primary" href="${pageContext.request.contextPath }/admin/network/edit/${objnetwork.id_network }"><i class="icon_pencil-edit_alt"></i></a></li>
                                	<%-- <li id="active-${objnetwork.id_network }">
                                	<c:choose>
                                		<c:when test="${objnetwork.active == 1 }">
                                			<a onclick="active(${objnetwork.id_network }, ${objnetwork.active})" class="btn btn-success" href="javascript:void(0)" ><i class="icon_check_alt2"></i></a>
                                		</c:when>
                                		<c:otherwise>
                                			<a onclick="active(${objnetwork.id_network }, ${objnetwork.active})" class="btn btn-warning" href="javascript:void(0)" ><i class="icon_upload"></i></a>
                                		</c:otherwise>
                                	</c:choose>
                                	</li> --%>
                                	<li><a class="btn btn-danger" onclick="return confirm('Bạn có muốn xóa không')" href="${pageContext.request.contextPath }/admin/network/del/${objnetwork.id_network}"><i class="icon_close_alt2"></i></a></li>
								...
								function  active(id, active) {
									$.ajax({
										url: '${pageContext.request.contextPath }/admin/network/active',
										type: 'POST',
										cache: false,
										data: {
												//Dữ liệu gửi đi
      ...
      <div class="pagination" style="margin-left: 23px;   margin-top: -31px;">           
			<div align="center" class="numbers">
			 <c:set value="${pageContext.request.contextPath }/admin/network/index/${page-1 }"  var="urlPre"></c:set>
                <c:set value="${pageContext.request.contextPath }/admin/network/index/${page+1 }" var="urlNext"></c:set>
                   <c:if test="${(page-1) > 0 }">
                    <a href="${urlPre }"><<</a>
                    </c:if>
                    <c:choose>
                    	<c:when test="${sumPage < 4 }">
                    		<c:forEach begin="1"  end="${sumPage }" var="i">
                      	 		<a <c:if test="${i == page }">class=active</c:if> href="${pageContext.request.contextPath }/admin/network/index/${i }">${i } <span class="sr-only"></span></a>
                      		</c:forEach>
                    	</c:when>
