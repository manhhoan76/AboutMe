    ...
    <%@include file="/templates/taglib.jsp" %>
      ...
      <section id="main-content">
		  ...
		  <div class="row">
				...
				<div style="margin-left: 16px; margin-top: -14px; margin-bottom: 5px; display: inline;">
                      ...
                      <form class="navbar-form" style="display: inline-flex;" action="${pageContext.request.contextPath }/admin/maxim/search">
                           <div>
                            <input class="form-control" placeholder="Search" type="text" name="key">
                      </div>
                        </form>
                       <a class="btn btn-success btn-sm"  href="${pageContext.request.contextPath }/admin/maxim/add" title="Bootstrap 3 themes generator" >Thêm</a>
				</div>
			</div>
			<c:choose>
				<c:when test="${msg == 1 }">
                      ...
                      <section class="panel">
                          <table class="table table-striped table-advance table-hover">
                           <tbody>
                              ...
                              <tr>
								 <td>${objMaxim.id_maxim }</td>
                                 <td>${objMaxim.author }</td>
                                 <td><img alt="${objMaxim.author }" class="img-rounded" style="width: 127px; height: 89px;" src="${pageContext.request.contextPath }/files/${objMaxim.picture }" /></td>
                                  <td>${objMaxim.content }</td>
                                 <td>${objMaxim.date_create }</td>
                                 <td>
                                  <div class="btn-group">
                                  <ul class="nav nav-tabs">
                                  	<li><a class="btn btn-primary" href="${pageContext.request.contextPath }/admin/maxim/edit/${objMaxim.id_maxim }"><i class="icon_pencil-edit_alt"></i></a></li>
                                	<li id="active-${objMaxim.id_maxim }">
                                	<c:choose>
                                		<c:when test="${objMaxim.active == 1 }">
                                			<a onclick="active(${objMaxim.id_maxim }, ${objMaxim.active})" class="btn btn-success" href="javascript:void(0)" ><i class="icon_check_alt2"></i></a>
                                		</c:when>
                                		<c:otherwise>
                                			<a onclick="active(${objMaxim.id_maxim }, ${objMaxim.active})" class="btn btn-warning" href="javascript:void(0)" ><i class="icon_upload"></i></a>
                                		</c:otherwise>
                                	</c:choose>
                                	</li>
                                	<li><a class="btn btn-danger" onclick="return confirm('Bạn có muốn xóa không')" href="${pageContext.request.contextPath }/admin/maxim/del/${objMaxim.id_maxim}"><i class="icon_close_alt2"></i></a></li>
								...
								function  active(id, active) {
									$.ajax({
										url: '${pageContext.request.contextPath }/admin/maxim/active',
										type: 'POST',
										cache: false,
										data: {
												//Dữ liệu gửi đi
      ...
      <div class="pagination" style="margin-left: 23px;   margin-top: -31px;">           
			<div align="center" class="numbers">
			 <c:set value="${pageContext.request.contextPath }/admin/maxim/index/${page-1 }"  var="urlPre"></c:set>
                <c:set value="${pageContext.request.contextPath }/admin/maxim/index/${page+1 }" var="urlNext"></c:set>
                   <c:if test="${(page-1) > 0 }">
                    <a href="${urlPre }"><<</a>
                    </c:if>
                    <c:choose>
                    	<c:when test="${sumPage < 4 }">
                    		<c:forEach begin="1"  end="${sumPage }" var="i">
                      	 		<a <c:if test="${i == page }">class=active</c:if> href="${pageContext.request.contextPath }/admin/maxim/index/${i }">${i } <span class="sr-only"></span></a>
                      		</c:forEach>
                    	</c:when>
